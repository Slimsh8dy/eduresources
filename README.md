...
Before switching, try a candidate with the real tutor prompt: `GET /probe?model=<model id>&q=<a question>` runs one non-streamed call and reports the text, how the call finished (`finish`), token usage, hidden-reasoning size and time taken. Add `&stream=1` to see the head of the raw event stream instead, and `&shape=messages` or `&shape=responses` to force the request shape. Only listed models are accepted. Each probe costs neurons like a question.

Streaming: on the live platform a streamed request in the Responses shape came back as a single empty chunk, so gpt-oss is asked without streaming (the whole answer arrives after a few seconds) while other models stream word by word. The `TUTOR_STREAM` variable overrides this: `responses` or `messages` streams in that shape, `off` never streams. `/health` reports the mode in use.
