...
Decision (14 Sep, midday): the tutor moves to OpenAI's open-weight gpt-oss-120b (`@cf/openai/gpt-oss-120b`) with reasoning effort low, a 1,500-character question cap (about 350–400 tokens), a 1,000-token reply cap (reasoning included), three questions a minute and ten a day per browser. The Worker asks in the Responses API shape (`input`/`instructions`/`reasoning.effort`/`max_output_tokens`) and falls back to chat messages; the daily count lives in the Cache API (best effort) and is reported to the page in the `meta` event, which shows the questions left. 59 tests pass. Verified on the live deployment (14 Sep, 12:50): `/health` reports the new limits; a non-streamed `/probe` with the real Kant question was accepted in the Responses shape and completed in 5 s with 43 characters of reasoning at low effort. Streamed `/ask` in the Responses shape returned a single empty `{"response":""}` chunk, so gpt-oss is now asked without streaming by default (`TUTOR_STREAM` can re-enable it once a working shape is found with `/probe?stream=1`). The page also softens `*italics*`, which gpt-oss uses despite the plain-prose instruction. 61 tests pass.

Addendum (14 Sep, morning): on the live deployment most questions began to fail with "No answer text in the model stream": Gemma 4 (now echoed by Workers AI as `…-external`) reasons privately and that reasoning used up the whole 1,600-token answer budget, so no answer text ever arrived (the one-word `/probe` still passed). The Worker was made model-agnostic: `CANDIDATE_MODELS` records, per model, how its private thinking is switched off (`reasoning_effort` + `chat_template_kwargs.enable_thinking` where the schema documents them; `/no_think` for Qwen3; nothing for plain instruct models); both `max_tokens` and `max_completion_tokens` are sent; schema rejections fall back step by step; `GET /probe?model=&q=` tries any listed model with the real tutor prompt and reports text, finish reason, usage and hidden-reasoning size; a stream that carried only reasoning now says so in its diagnostic. The tutor page reads the model's name from `/health`. 55 tests pass. Not verified here: which of the knobs the live platform honours for each model — that is what `/probe` is for.
